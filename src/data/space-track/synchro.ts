...
import { SPACE_TRACK_GP_CACHE_KEY, SPACE_TRACK_GP_CACHE_TTL } from './constants';
...
export const getSyncGeneralPerturbationsOrGetFromApi = async (
    ...
    try {
        ...
        if (freshData instanceof Error) {
            logger.error('Erreur lors de la récupération des données depuis Space-Track', {
        ...
        await redisClient.set(SPACE_TRACK_GP_CACHE_KEY, freshData, SPACE_TRACK_GP_CACHE_TTL);
        logger.info(`Données stockées dans Redis avec expiration de ${SPACE_TRACK_GP_CACHE_TTL}s`, {
 ...
 */
export const invalidateGeneralPerturbationsCache = async (): Promise<boolean> => {
    try {
